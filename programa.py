...
while jogada < 12:
    ...
    while jogando:
        ...
        if acao == "0":
            ...
            r = int(input(">"))
            if len(dados) > 0:
                lista = funcoes.guardar_dado(dados, guardados, r)
                dados = lista[0]
                guardados = lista[1]
        ...
        if cartela_de_pontos["regra_simples"]["1"] != -1 and cartela_de_pontos["regra_simples"]["2"] != -1 and cartela_de_pontos["regra_simples"]["3"] != -1:
            jogando = False
            jogada = 12

    jogada += 1

pontuacao_final = cartela_de_pontos["regra_simples"]["1"] + cartela_de_pontos["regra_simples"]["2"] + cartela_de_pontos["regra_simples"]["3"] + cartela_de_pontos["regra_simples"]["4"] + cartela_de_pontos["regra_simples"]["5"] + cartela_de_pontos["regra_simples"]["6"]
